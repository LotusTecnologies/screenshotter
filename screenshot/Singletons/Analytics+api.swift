...
extension AnalyticsCartPressedConfirmOrder {
  ...
  static func trackCartPressedConfirmOrder(cart:Cart?,  result:AnalyticsCartPressedConfirmOrderResult ) {
      ...
      AnalyticsTrackers.autoGeneratedCodeApi.kochava.track(key, properties: properties, sendEvenIfAdvertisingTrackingIsOptOut: false)
      AnalyticsTrackers.autoGeneratedCodeApi.amplitude.track(key, properties: properties, sendEvenIfAdvertisingTrackingIsOptOut: false)
      AnalyticsTrackers.autoGeneratedCodeApi.pushwoosh.track(key, properties: properties, sendEvenIfAdvertisingTrackingIsOptOut: false)
      ...
      cart?.items?.forEach({ (i) in
            if let i = i as? CartItem {
                ...
                AnalyticsTrackers.autoGeneratedCodeApi.pushwoosh.track(subItemKey, properties: properties, sendEvenIfAdvertisingTrackingIsOptOut: false)
            }
      })


      Analytics.debugShowLoggedAnalytics(eventName: key, properties: properties, destinations:["appsee", "kochava", "amplitude", "pushwoosh" ])
      //edit properties after sent them to supress complier warning if unused
      properties["_______"] = ""
...
extension AnalyticsFeatureRecoveryAppeared {
  ...
  static func trackFeatureRecoveryAppeared(product:Product? ) {
...
typealias AnalyticsFeatureRecoveryEmailPresented = Analytics
extension AnalyticsFeatureRecoveryEmailPresented {
    
  static func trackFeatureRecoveryEmailPresented(product:Product? ) {
      let key = "feature-recovery-email-presented"
      var properties:[String:Any] = [:]
      propertiesForAllEvents().forEach { properties[$0] = $1 }
...
extension AnalyticsOnboardingFacebookStarted {
  ...
  static func trackOnboardingFacebookStarted() {
...
extension AnalyticsOnboardingFacebookSuccess {
  ...
  static func trackOnboardingFacebookSuccess(isReturning:Bool? ) {
...
extension AnalyticsProductRemovedFromCart {
  ...
  static func trackProductRemovedFromCart(cartItem:CartItem? ) {
      ...
      Analytics.debugShowLoggedAnalytics(eventName: key, properties: properties, destinations:["appsee", "amplitude", "pushwoosh" ])
      //edit properties after sent them to supress complier warning if unused
      properties["_______"] = ""
...
extension AnalyticsAppOpenedFromPushNotification {
  enum AnalyticsAppOpenedFromPushNotificationSource : String{
    case `favorited` = "favorited"
    case `tapped` = "tapped"
    }
    
  static func trackAppOpenedFromPushNotification(source:AnalyticsAppOpenedFromPushNotificationSource ) {
...
extension AnalyticsScreenshotCreatedPerCategory {
  ...
  static func trackScreenshotCreatedPerCategory(screenshot:Screenshot?,  category:String ) {
      ...
      Analytics.debugShowLoggedAnalytics(eventName: key, properties: properties, destinations:["pushwoosh" ])
      //edit properties after sent them to supress complier warning if unused
      properties["_______"] = ""
