...
extension AnalyticsAppSentLocalPushNotification {
  ...
  static func trackAppSentLocalPushNotification(success:Bool,  localizedDescription:String? ) {
      let key = "app sent local push notification"
      var properties:[String:Any] = [:]
      propertiesForAllEvents().forEach { properties[$0] = $1 }


      properties["success"] = NSNumber.init(value: success)
...
typealias AnalyticsFeatureLowerPricesAppeared = Analytics
extension AnalyticsFeatureLowerPricesAppeared {
    
  static func trackFeatureLowerPricesAppeared(product:Product?,  secondsSinceLeftApp:Int? ) {
      let key = "Sale recovery prompt displayed"
      var properties:[String:Any] = [:]
      propertiesForAllEvents().forEach { properties[$0] = $1 }
...
typealias AnalyticsFeatureLowerPricesClicked = Analytics
extension AnalyticsFeatureLowerPricesClicked {
    
  static func trackFeatureLowerPricesClicked(product:Product? ) {
      let key = "Sale recovery prompt - Item tapped"
      var properties:[String:Any] = [:]
      propertiesForAllEvents().forEach { properties[$0] = $1 }
...
typealias AnalyticsFeatureLowerPricesDidNotAppear = Analytics
extension AnalyticsFeatureLowerPricesDidNotAppear {
  enum AnalyticsFeatureLowerPricesDidNotAppearReason : String{
    case `notLoadedYet` = "not loaded yet"
    case `notEnoughCheaperProducts` = "not enough cheaper products"
    }
    
  static func trackFeatureLowerPricesDidNotAppear(product:Product?,  secondsSinceLeftApp:Int?,  reason:AnalyticsFeatureLowerPricesDidNotAppearReason? ) {
      let key = "Sale recovery prompt - did not appear"
      var properties:[String:Any] = [:]
      propertiesForAllEvents().forEach { properties[$0] = $1 }
...
typealias AnalyticsFeatureLowerPricesDismiss = Analytics
extension AnalyticsFeatureLowerPricesDismiss {
    
  static func trackFeatureLowerPricesDismiss(product:Product? ) {
      let key = "Sale recovery prompt dismissed"
      var properties:[String:Any] = [:]
      propertiesForAllEvents().forEach { properties[$0] = $1 }
...
typealias AnalyticsInboxOpened = Analytics
extension AnalyticsInboxOpened {
    
  static func trackInboxOpened(tab:String,  unread:Int,  total:Int ) {
      let key = "inbox-opened"
      var properties:[String:Any] = [:]
      propertiesForAllEvents().forEach { properties[$0] = $1 }
      ...
      properties["unread"] = NSNumber.init(value: unread)


      properties["total"] = NSNumber.init(value: total)
...
typealias AnalyticsInboxTappedMessage = Analytics
extension AnalyticsInboxTappedMessage {
    
  static func trackInboxTappedMessage(tracking:[String:String],  action:String?,  isExpired:Bool,  order:Int? ) {
      let key = "inbox-tapped_message"
      var properties:[String:Any] = [:]
      propertiesForAllEvents().forEach { properties[$0] = $1 }
      ...
      properties["isExpired"] = NSNumber.init(value: isExpired)
...
typealias AnalyticsMatchsticksFilterCategory = Analytics
extension AnalyticsMatchsticksFilterCategory {
    
  static func trackMatchsticksFilterCategory(gender:String?,  category:String? ) {
      let key = "matchsticks-filter-category"
      var properties:[String:Any] = [:]
      propertiesForAllEvents().forEach { properties[$0] = $1 }
...
typealias AnalyticsMatchsticksFilterGender = Analytics
extension AnalyticsMatchsticksFilterGender {
    
  static func trackMatchsticksFilterGender(gender:String?,  category:String? ) {
      let key = "matchsticks-filter-gender"
      var properties:[String:Any] = [:]
      propertiesForAllEvents().forEach { properties[$0] = $1 }
...
typealias AnalyticsMatchsticksFilterNoResults = Analytics
extension AnalyticsMatchsticksFilterNoResults {
    
  static func trackMatchsticksFilterNoResults(gender:String?,  category:String? ) {
      let key = "matchsticks-filter-noResults"
      var properties:[String:Any] = [:]
      propertiesForAllEvents().forEach { properties[$0] = $1 }
...
extension AnalyticsProductOpened {
  ...
  enum AnalyticsProductOpenedFromPage : String{
    ...
    case `saleRecoveryPrompt` = "Sale recovery prompt"
    }
...
extension AnalyticsProductFavorited {
  enum AnalyticsProductFavoritedPage : String{
    ...
    case `cart` = "cart"
    case `similarItemsPopup` = "Similar Items Popup"
    }
...
typealias AnalyticsScreenshotMatching = Analytics
extension AnalyticsScreenshotMatching {
    
  static func trackScreenshotMatching(screenshot:Screenshot?,  screenshotMatchId:String,  shoppableIdArray:String ) {
      let key = "Started screenshot matching session"
      var properties:[String:Any] = [:]
      propertiesForAllEvents().forEach { properties[$0] = $1 }
      ...
      properties["screenshotMatchId"] = screenshotMatchId


      properties["shoppableIdArray"] = shoppableIdArray
      ...
      Analytics.debugShowLoggedAnalytics(eventName: key, properties: properties, destinations:["amplitude" ])
      //edit properties after sent them to supress complier warning if unused
      properties["_______"] = ""
...
typealias AnalyticsSearchResultEmpty = Analytics
extension AnalyticsSearchResultEmpty {
    
  static func trackSearchResultEmpty() {
      let key = "search-result-empty"
      var properties:[String:Any] = [:]
      propertiesForAllEvents().forEach { properties[$0] = $1 }
...
typealias AnalyticsSearchResultTappedBrand = Analytics
extension AnalyticsSearchResultTappedBrand {
    
  static func trackSearchResultTappedBrand(brand:String ) {
      let key = "search-result-tapped-brand"
      var properties:[String:Any] = [:]
      propertiesForAllEvents().forEach { properties[$0] = $1 }
...
typealias AnalyticsSearchResultTappedProduct = Analytics
extension AnalyticsSearchResultTappedProduct {
    
  static func trackSearchResultTappedProduct(product:String ) {
      let key = "search-result-tapped-product"
      var properties:[String:Any] = [:]
      propertiesForAllEvents().forEach { properties[$0] = $1 }
...
typealias AnalyticsSearchTappedCategory = Analytics
extension AnalyticsSearchTappedCategory {
    
  static func trackSearchTappedCategory(category:String ) {
      let key = "search-tapped-category"
      var properties:[String:Any] = [:]
      propertiesForAllEvents().forEach { properties[$0] = $1 }
...
typealias AnalyticsSearchTapped = Analytics
extension AnalyticsSearchTapped {
    
  static func trackSearchTapped() {
      let key = "search-tapped"
      var properties:[String:Any] = [:]
      propertiesForAllEvents().forEach { properties[$0] = $1 }
...
extension AnalyticsShoppableMatching {
  ...
  static func trackShoppableMatching(shoppable:Shoppable?,  screenshotMatchId:String,  shoppableMatchId:String,  productIdArray:String,  augmentedFeed:String,  augmentedGender:String?,  augmentedCurrency:String? ) {
      ...
      properties["shoppableMatchId"] = shoppableMatchId
