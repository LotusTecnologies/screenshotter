...
// created on: Wed May  2 09:55:02 IDT 2018
// created from: betterCode -  - 6e0bd7f  WARNING BRANCH WAS DIRTY 
...
typealias AnalyticsCartCvvEnteredErrorEmpty = Analytics
extension AnalyticsCartCvvEnteredErrorEmpty {
    
  static func trackCartCvvEnteredErrorEmpty(cart:Cart? ) {
      let key = "cart-cvv-entered-error-empty"
      var properties:[String:Any] = [:]
      if let cart = cart {
          propertiesFor(cart).forEach { properties[$0] = $1 }
      }
      ...
      let subItemKey = "cart-cvv-entered-error-empty-perItem"
      cart?.items?.forEach({ (i) in
            if let i = i as? CartItem {
